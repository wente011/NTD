...
an<- mt %>% group_by(Year,`UZA Name`,UZA,pop.sa,pop.uza,area,MT) %>%
        summarize(upt=sum(upt,na.rm=T),vrm=sum(vrm,na.rm=T)) %>% ungroup() %>%
        mutate(upt.vrm=upt/vrm,MT=as.factor(MT)) %>% group_by(Year) %>%
        mutate(
...
gg7<- an %>% filter(MT != "Oth") %>% ggplot(aes(x=Year,y=upt.vrm,color=MT)) + geom_line() + 
...
gg8<-an %>% filter(MT != "Oth") %>% ggplot(aes(x=Year,y=pct.rank,color=MT)) + geom_line() + 
                                    scale_y_continuous("Percentile UPT/VRM", 
                                                        breaks=seq(0,1,.1),label=scales::percent) + 
                                    scale_x_continuous(breaks=seq(2002,2019,1)) + 
                                     theme(axis.text.x = element_text(angle=45)) + 
                                     ggtitle("Percentile Ranking of METC UPT/VRM, (n=389 UZA)")
